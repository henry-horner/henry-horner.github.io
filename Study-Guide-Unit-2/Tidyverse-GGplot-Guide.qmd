...
```{r}
install.packages("tidyverse")   ## Installs the package (only needed once)
library(tidyverse)  ## Loads the package (needed everytime we reopen/reload our project)
```

- Read the CSV file into a tibble dataframe to manipulate later
```{r}
dat.Birds = read_csv("Table_S1.csv")   ## read_csv() form the `tidyverse` package makes it a tibble automatically
```
...
```{r}
dat.Birds = dat.Birds |>                            ## Reset dat.Birds to clean version and pass it through
            rename_with(~ gsub(" ", ".", .x)) |>    ## Removing all " " and replacing with "."
            drop_na(Order)              ## checking and removing all NA's
```
