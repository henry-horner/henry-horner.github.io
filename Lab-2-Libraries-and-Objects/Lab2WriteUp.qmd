...
**Solution:**
We use **`install:packages()`** on `equisse` and use `eval: false` to ensure it wont be installed everytime.
```{r}
#| size: "scriptsize"
#| code-fold: FALSE
#| eval: false
...
**Solution:**
No there is not a `demo()` or `vignette()` for `esquisse`
```{r}
#| size: "scriptsize"
#| code-fold: FALSE
#| eval: false
...
**Solution:**
`esquisse` is a package that gives us the ability to create "ggplot2" figures interactively using available data frames.*
...
*Since there is both a character type and a numeric in the same vector, the vector converts all types inside of it to character since character types override numerics.*